...
void ui_draw_text(TTF_Font* font, const char* text,
    ...
    float tw = (float)rgba->w, th = (float)rgba->h;
...
// ─────────────────────────────────────────────────────────────────────────────
// Slider — returns updated value, handles mouse drag live
// ─────────────────────────────────────────────────────────────────────────────
...
static float draw_slider(TTF_Font* font,
    ...
    gl_fill_rect(x + w*t - 6, ty - 5, 12, 16, 0.78f, 0.86f, 1.0f, 1.0f);
    ...
    if (mb & SDL_BUTTON(SDL_BUTTON_LEFT)) {
        if ((float)mx >= x-4 && (float)mx <= x+w+4 &&
            ...
            float nt = ((float)mx - x) / w;
            if (nt < 0) nt = 0; if (nt > 1) nt = 1;
            return vmin + nt * (vmax - vmin);
        }
    }
    return val;
}
...
TTF_Font* ui_init(const char* font_path, int size) {
    ...
    if (!font) printf("Font load error (%s): %s\n", font_path, TTF_GetError());
    return font;
}
...
void ui_draw_menu(TTF_Font* font, int win_w, int win_h, Button btns[3]) {
    ...
    for (int i = 0; i < 3; i++) {
        btns[i].x = bx; btns[i].y = by_start + i * 70.0f;
        btns[i].w = bw; btns[i].h = bh;
    ...
    if (font) {
        int fw, fh; TTF_SizeUTF8(font, "F1 for controls  |  O: toggle orbits", &fw, &fh);
        ui_draw_text(font, "F1 for controls  |  O: toggle orbits",
...
void ui_draw_editor(TTF_Font* font, World* world, EditorState* es,
    ...
    for (int i = 0; i < world->count; i++) {
        ...
        if (font) ui_draw_text(font, row_label, 16, ry+4, ir, ig, ib, 1.0f, win_w, win_h);
    }
    ...
    // ── Scissor & scroll ─────────────────────────────────────────────────────
    float scissor_y = props_y + 10.0f;

    glEnable(GL_SCISSOR_TEST);
    glScissor(0, 50, (int)PANEL_W, (int)(win_h - scissor_y - 50));

    float sy = props_y + 10.0f - es->scroll_y;
    ...
    // ════════════════════════════════════════════════════════════════════════
        ...
        if (font) ui_draw_text(font, disp, sx+4, sy+4, 0.31f,0.63f,1.0f,1.0f, win_w,win_h);
        sy += 32.0f;

        // Object type selector
        // FIX: type_labels order matches enum (OBJ_PLANET=0, OBJ_STAR=1, OBJ_MOON=2)
        if (font) ui_draw_text(font, "Type:", sx, sy, 0.51f,0.51f,0.59f,1.0f, win_w,win_h);
        sy += 20.0f;
        float type_w = (sw - 8.0f) / 3.0f;
        const char* type_labels[3] = {"Planet", "Star", "Moon"}; // matches OBJ_PLANET=0, OBJ_STAR=1, OBJ_MOON=2
        for (int t = 0; t < 3; t++) {
            float tx = sx + t*(type_w+4.0f);
            int sel = (p->obj_type == t);
            // Colour scheme: Planet=blue, Star=yellow/orange, Moon=grey/purple
            float fr = sel ? (t==1?0.28f:t==0?0.12f:0.10f) : 0.06f;
            float fg = sel ? (t==1?0.22f:t==0?0.24f:0.14f) : 0.08f;
            float fb = sel ? (t==1?0.06f:t==0?0.47f:0.28f) : 0.18f;
            gl_fill_rect(tx, sy, type_w, 26, fr,fg,fb,0.9f);
            float br = sel ? (t==1?1.0f:t==0?0.31f:0.45f) : 0.24f;
            float bg = sel ? (t==1?0.85f:t==0?0.55f:0.55f): 0.31f;
            float bb = sel ? (t==1?0.20f:t==0?1.0f :0.90f) : 0.55f;
        ...
        if (font) {
            ...
            ui_draw_text(font, par_buf, sx+4, sy+4, 0.60f,0.80f,1.0f,1.0f, win_w,win_h);
        }
        sy += 34.0f;
    ...
    // ════════════════════════════════════════════════════════════════════════
        ...
        // ── Rings ────────────────────────────────────────────────────────────
        {
            ...
            char ring_buf[40];
            snprintf(ring_buf,sizeof(ring_buf),"Rings: %s  (click to toggle)",hr?"ON":"OFF");
        ...
        if (p->has_rings) {
            ...
            p->ring_tilt  = draw_slider(font,sx,sy,sw,"Ring tilt",   p->ring_tilt, 0.0f,180.0f, win_w,win_h); sy+=40.0f;

            // Live update particle distances when inner/outer changes
            if (p->ring_particles) {
...
static void add_blank_planet(World* world, EditorState* es) {
    ...
    p->ring_inner = 1.3f; p->ring_outer = 2.1f; p->ring_tilt = 0.0f;
...
void ui_editor_click(int x, int y, World* world, EditorState* es,
    ...
    float list_y  = editor_list_y();
    float after_list = list_y + world->count * 26.0f + 8.0f;  // after "Objects:" label+24
    float btn1_y  = editor_btn1_y(world);
    float btn2_y  = editor_btn2_y(world);
    float tab_y   = editor_tab_y(world);
    float props_y = editor_props_y(world);
    ...
    // Properties area (scrolled)
    if ((float)y >= props_y + 10.0f && (float)y < save_y) {
        float sy = props_y + 10.0f - es->scroll_y;
...
void save_planets(World* world, const char* filename) {
    ...
    for (int i = 0; i < world->count; i++) {
        ...
        fprintf(f, "%s,%.4f,%.4f,%.4f,%.4f,%.4f,%s,%d,%.3f,%.3f,%.3f,"
                ...
                p->ring_inner, p->ring_outer, p->ring_tilt,
                (int)p->obj_type, par);
    }
    fclose(f);
}